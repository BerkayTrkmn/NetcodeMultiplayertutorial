...
namespace HelloScripts
{
    public static class Library
    {
        ...
        /// <summary>
        /// Change Activity of gameovject inside children
        /// </summary>
        /// <typeparam name="T">Destroyed Component</typeparam>
        /// <param name="parentTrans"></param>
        public static void ChangeActivityAllChild(this Transform parentTrans, bool activity)
        {
        ...
        /// <param name="parentTrans"></param>
        /// <param name="activity"></param>
        public static void ChangeActivityAllListOfComponents<T>(this List<T> parentTrans, bool activity) where T : Component
        {
            for (int i = 0; i < parentTrans.Count; i++)
            {
              parentTrans[i].gameObject.SetActive(activity);
            }
        ...
        /// 
        /// </summary>
        /// <param name="mainVector">bu vectorün hangi değeri(x,y) daha büyükse ona göre</param>
        /// <param name="checkingVector"></param>
        /// <returns></returns>
        private static bool IsTwoVectorSameDirection(Vector2 mainVector, Vector2 checkingVector)
        {
            bool isXBig = mainVector.x > mainVector.y ? true : false;
            if (isXBig)
            {
                if ((mainVector.x < 0 && checkingVector.x < 0) || (mainVector.x > 0 && checkingVector.x > 0))
                    return true;
                else return false;
            }
            else
            {
                if ((mainVector.y < 0 && checkingVector.y < 0) || (mainVector.y > 0 && checkingVector.y > 0))
                    return true;
                else return false;
            }
        }
        ...
        public static List<T> Shuffle<T>(this List<T> _list)
        {
            ...
            {
                T temp = newList[i];
                int randomIndex = UnityEngine.Random.Range(i, newList.Count);
